 ...
 * 
 * public class DeleteBatchbyIDStepDef { RequestSpecification req;
 * 
 * RequestSpecification res; Response response;
 * 
 * @Given("A Service with URL \\(Delete Batch)") public void
 * a_service_with_url_delete_batch() { req =new RequestSpecBuilder().setBaseUri(
 * "https://lms-backend-service.herokuapp.com/lms/")
 * .setContentType(ContentType.JSON).build(); res=given().spec(req); }
 * 
 * @When("DELETE request is made by passing ID as a parameter") public void
 * delete_request_is_made_by_passing_id_as_a_parameter() {
 * response=res.when().delete("/batches/582").then().extract().response(); }
 * 
 * @Then("the API call got success with status code {int}") public void
 * the_api_call_got_success_with_status_code(Integer int1) {
